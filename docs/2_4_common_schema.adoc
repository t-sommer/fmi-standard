...
_[This flag can be set to `false` for variables where discrete states are directly updated when assigned (_ latexmath:[\mathbf{x}_{d} := f(\mathbf{x}_{d},\mathbf{u}_{c+d})] _instead of_ latexmath:[\mathbf{x}_{d} := f({}^\bullet\mathbf{x}_{d},\mathbf{u}_{c+d})] _)._
_If an <<output>> depends on this <<input>> and on discrete states, then an algebraic loop could be formed with such an <<input>>._
...
May only be present for a <<state,continuous state>>. +
