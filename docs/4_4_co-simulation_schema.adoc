...
The attributes in the following table are defined on top of the <<common-capability-flags, common attributes>> and have the following meaning (all attributes are optional with exception of <<modelIdentifierCS>>):
...
|[[modelIdentifierCS,`modelIdentifier`]]`modelIdentifier`
...
<<modelIdentifierCS>> is also used as name of the static library or DLL/SharedObject.
