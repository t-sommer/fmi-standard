...
The mode <<StepMode>> is used to compute the values of all variables between communication points and events and to advance time in the FMU.
...
* Update previous values of non-clocked, discrete-time states: latexmath:[{}^\bullet\mathbf{x}_{d-k} := \mathbf{x}_{d-k}].
...
The FMU enters <<IntermediateUpdateMode>> by calling <<fmi3IntermediateUpdateCallback>> within <<StepMode>> and returns to <<StepMode>> when the callback function returns.
...
* If <<intermediateStepFinished, `intermediateStepFinished = fmi3False`>>, the intermediate output values of the FMU that the importer inquires with <<get-and-set-variable-values,`fmi3Get{VariableType}`>> resulting from tentative internal solver states and may still change for the same <<intermediateUpdateTime>> _[e.g., if the solver deems the tentative continuous-time state to cause an unacceptable approximation error, it may go back in time and try to re-estimate the continuous-time state using smaller internal time steps]_. +
