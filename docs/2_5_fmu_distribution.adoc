...
binaries                            // directory containing the binaries (optional)
   x86_64-windows                   // binaries for Windows on Intel 64-bit
      model.dll                     // shared library of the FMI implementation
      <other DLLs>                  // the DLL can load other DLLs
   x86-linux                        // binaries for Linux on Intel 32-bit
      model.so                      // shared library of the FMI implementation
   aarch32-linux                    // binaries for Linux on ARM 32-bit
      model.so                      // shared library of the FMI implementation
   x86_64-darwin                    // binaries for macOS
      model.dylib                   // shared library of the FMI implementation
...
The header files `fmi3PlatformTypes.h`, `fmi3FunctionTypes.h` and `fmi3Functions.h` are supplied by the importer to potentially reflect their specific platform and compiler tool set.
...
The importer chooses the matching <<BuildConfiguration>> based on the <<platform>> attribute.
...
All <<platform>> attributes defined in the <<BuildConfiguration>> elements must be unique.
...
If an FMU is loaded from a shared library, the containing folder must be extracted completely retaining all files and directories.
