...
The importer chooses the matching <<BuildConfiguration>> based on the <<platform>> and <<modelIdentifierME>> attributes.
...
|`modelIdentifier`
|The attribute `modelIdentifier` of the <<modelIdentifierME,`<ModelExchange>`>>, <<modelIdentifierCS,`<CoSimulation>`>> or <<modelIdentifierSE,`<ScheduledExecution>`>> elements this build configuration is associated with.
