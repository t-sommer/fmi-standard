...
The attribute in the following table is defined on top of the <<common-capability-flags, common attributes>> and have the following meaning (all attributes are optional with exception of <<modelIdentifierSE>>):
...
|[[modelIdentifierSE,`modelIdentifier`]]`modelIdentifier`
...
<<modelIdentifierSE>> is also used as name of the static library or DLL/SharedObject.
