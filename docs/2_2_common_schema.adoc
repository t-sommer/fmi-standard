...
The variable sub type `fmi3Clock` provides additional attributes for defining clocks.
...
|clockType
|The type of clocks is defined based on the mandatory attribute `clockType`.
If the properties of a clock adhere to synchronous clock theory it is defined with clockType = `"synchronousTime"`.
As an additional option for FMI for Co-Simulation discrete parts (that do not necessarily apply to synchronous clock theory) or continuous parts of the FMU can be associated to model partitions via clocks whose ticks define the sampling points (i.e. communication points) for the variables of these model partitions.
...
It is not allowed to include clocks of different `clockType` in one FMU.
...
It is possible to define multiple clocks with the same priority.
...
_[For periodic clocks it is recommended to derive the priorities based on a rate monotonic scheduling scheme (smallest period leads to highest priority (smallest priority value)).]_

`priority` is a required attribute.

|periodic
|Clocks can be periodic or non-periodic. If a clock is periodic the attribute `periodic` equals "true".
...
It is not allowed to set `strict` to "true" if `periodic` equals "false".

|intervalCounter, shiftCounter, resolution
|The interval of output or input periodic clocks is defined with unsignedLong valued `intervalCounter` and `resolution`
...
The attributes `intervalCounter`, `shiftCounter` and `resolution` are interval start values for periodic clocks and must not be used together with non-periodic clocks.
If `strict` equals "true" it is required to provide values for `intervalCounter`, `shiftCounter` and `resolution` in the modelDescription.
...
It is not possible to use a variable of type `Clock` in arrays, since array elements do not have individual valueReferences and the access to clocks requires a unique valueReference information.
...
- `"input"`: the variable defines an input clock that is controlled by the master.
Variability must be "clock".
Only a variable of type "Clock" can have this causality.

- `"output"`: the variable defines an output clock that is controlled by the FMU.
Variability must be "clock".
Only a variable of type "Clock" can have this causality.
...
|`clockReference`
|The optional attribute `clockReference` is used to define the clock(s) this variable is assigned to.
...
Clock (clockType equals `synchronousTime`): If present, the variable is a clocked variable associated uniquely with the clock referenced by valueReference and defined in element ModelVariables.
It is not possible to associate more than one clock to a variable.

Communication Point Clock (clockType equals `communicationPoint`): If present, the variable is associated with the clock referenced by valueReference and defined in element `ModelVariables`.
It is possible to associate multiple clocks to a variable. Variables that are assigned to a Communication Point Clock are not necessarily "clocked" in the sense of synchronous time clock theory. Such variables can also be Continuous-time or Discrete-time variables.

If output clocks and input clocks of clockType equals `communicationPoint` are defined in the modelDescription.xml it is possible to define a tick relationship from a output clock to an aperiodic input clock based on `clockReference` (<<clock-relationships-for-communication-point-clocks>>).
This is done for aperiodic input clocks that have the clockType = `communicationPoint` via providing a list of valueReferences of output clocks of the same FMU.
If a output clock of this valueReference list ticks the master has to create a tick for the associated aperiodic input clock at the same time instant.
It is not allowed to combine output clocks with periodic or strict periodic input clocks based on `clockReference`.

The attribute clockReference must not be used if `causality` equals "output" for a variable.
...
Variables with causality "parameter" must not be marked with `intermediateAccess` equals true.
...
>|_[green]#(18)#_
|_clock_
|_Variable that defines a clock_
|====
...
|`DiscreteStates`
|Ordered list of all exposed discrete states, in other words a list of Variable indices. Every corresponding Variable may have attribute previous defined. Discrete states must have a `clockReference` attribute.
...
(3) discrete are exposed without previous values.
...
Attribute dependencies defines the dependencies of the discrete states from the known inputs, continuous-time states and previous discrete-time states.
----------*TODO*-------------
|====
...
*TODO* is it helpful to include also clock dependencies here --> i.e. changing this variable will lead to a output clock tick.*TODO*
