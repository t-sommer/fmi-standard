...
Only for variables with <<causality>> = <<input>> and <<variability>> = <<continuous>>, and continuous states.
...
+
_[<<fmi3Discard,`fmi3Status = fmi3Discard`>> should be returned if the FMU rejects any of the continuous state values because they, for example, violate min/max value restrictions.]_
...
** _for <<dynamic-state-selection,dynamic state selection>>: The FMU checks whether the dynamically selected continuous states are still numerically appropriate._
...
** _for projection: FMUs that represent a DAE can project their continuous states onto the constraint manifold in <<fmi3UpdateDiscreteStates>>._

+
_The new mapping or projection values might require a reinitialization of continuous states, which must be signaled to the importer by returning from <<fmi3UpdateDiscreteStates>> with <<valuesOfContinuousStatesChanged,`valuesOfContinuousStatesChanged = fmi3True`>>._
