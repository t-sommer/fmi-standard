...
Common modes are defined in <<common-state-machine>>, such as super modes <<FMUStateSettable>> and <<Initialized>>, modes <<Instantiated>>, <<ConfigurationMode>>, <<ReconfigurationMode>>, <<InitializationMode>>, <<EventMode>>, and <<Terminated>>.
...
_As soon as an event occurs (<<fmi3EnterEventMode>> was called), going back in time is impossible, because <<fmi3EnterEventMode>> / <<fmi3UpdateDiscreteStates>> can only compute the next discrete-time state, not the previous one.]_

Function <<get-and-set-variable-values,`fmi3Set{VariableType}`>>::
Only for variables with <<causality>> = <<input>> and <<variability>> = <<continuous>>, and continuous-time states.
...
----
+
Set new continuous-time state values.
+
* Argument `continuousStates` contains the new values for each <<state,continuous-time state>>.
...
+
_[<<fmi3Discard,`fmi3Status = fmi3Discard`>> should be returned if the FMU rejects any of the continuous-time state values because they, for example, violate min/max value restrictions.]_

Function <<fmi3GetContinuousStates>>::
Returns the current continuous-time state vector.
...
----
+
Returns the first-order derivatives with respect to the independent variable (usually time) of the continuous-time states.
+
* Return argument `derivatives` contains the derivatives for each <<state,continuous-time state>> with the same convention for the order as defined for <<fmi3SetContinuousStates>>.
...
The importer must have set valid values for <<time>>, <<continuous>> <<input,`inputs`>> and <<continuous>>-time <<state,states>> prior to calling this function to evaluate latexmath:[\mathbf{f}_{\mathit{comp}}] with valid right-hand side data.
+
--
...
This is usually performed by an iteration where time is varied and continuous-time <<state>> variables are determined by interpolation.
Function <<fmi3CompletedIntegratorStep>> must be called after this <<state event>> location procedure.
The intended purpose of the function call is to indicate to the FMU that at this stage all <<input,`inputs`>> and continuous-time <<state>> variables have valid (accepted) values.
...
** _for <<dynamic-state-selection,dynamic state selection>>: The FMU checks whether the dynamically selected continuous-time states are still numerically appropriate._
...
----
+
This function changes the mode to <<EventMode>>.
+
