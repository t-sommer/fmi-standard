...
_[These macros can be defined differently in a target specific variant of `fmi3Functions.h` to adjust them to the requirements of the supported compilers and platforms of the importer, e.g. one can remove the use of the `FMI3_FUNCTION_PREFIX` macro in the `fmi3Function.h` file of the importer to compile function names without prefix for building shared objects (DLL/SO).]_
...
_[For example, the Windows API only supports full path-names of a file up to 260 characters (see: https://msdn.microsoft.com/en-us/library/aa365247%28VS.85%29.aspx).]_
...
Using this pointer, data may be transferred between the importer and callback functions the importer provides with the <<FMUStateSettable,instantiation functions>>.
...
When detecting illegal arguments or a function call not allowed in the current mode according to the respective state machine, the FMU must return <<fmi3Error>>.
...
[[fmi3Fatal,`fmi3Fatal`]]
`fmi3Fatal`::
The state machine of all instances of the model is irreparably corrupted.
...
Restrictions for setting and getting of variables with certain types, causalities and variabilities are defined in the state machine and mode descriptions (see <<common-state-machine>> for common modes, <<state-machine-model-exchange>> for Model Exchange, <<state-machine-co-simulation>> for Co-Simulation, and <<state-machine-scheduled-execution>> for Scheduled Execution).
...
The FMU has internal data representing its entire state.
This internal FMU state consists especially of the values of the continuous-time states, discrete-time states, iteration variables, <<parameter>> values, <<input>> values, delay buffers, file identifiers, and FMU internal status information.
Depending on the FMI type, only a subset of this data is directly accessible via the FMI C-API.
With the functions of this section, the entire internal FMU state can be stored and reapplied to continue the simulation from this FMU state.
...
* _For nonlinear Kalman filters: get the FMU state just before initialization; in every sample period, set new continuous-time states from the Kalman filter algorithm based on measured values; integrate to the next sample instant and inquire the predicted continuous-time states that are used in the Kalman filter algorithm as basis to set new continuous-time states._

* _For nonlinear model predictive control: get the FMU state just before initialization; in every sample period, set new continuous-time states from an observer, initialize and get the FMU state after initialization._
...
This function restores the FMU state provided by <<FMUState>>.
...
.2+|<<input>>
|`changing`
|[[changing-aperiodic-clock]]<<fmi3SetClock>> in <<EventMode>>,
<<fmi3ActivateModelPartition>> in <<ClockActivationMode>>,
...
|`countdown`
|[[countdown-aperiodic-clock]]<<fmi3SetClock>> in <<EventMode>>,
<<fmi3ActivateModelPartition>> in <<ClockActivationMode>>,
...
|The delay for the first Clock tick relative to latexmath:[t_{\mathit{start}}]. latexmath:[T_{\mathit{shift}}] is defined differently for the different Clock types, and can be set with <<fmi3SetShiftDecimal>> or retrieved from the FMU with <<fmi3GetShiftDecimal>> as floating point value, or as rational number using <<fmi3SetShiftFraction>> or <<fmi3GetShiftFraction>>.

|latexmath:[T_{\mathit{interval, i}}]
|The time interval until the next Clock tick, defined differently for the different Clock types.
latexmath:[T_{\mathit{interval, i}}] can be set with <<fmi3SetIntervalDecimal>> or retrieved with <<fmi3GetIntervalDecimal>> as floating point value, or as rational number using <<fmi3SetIntervalFraction>> or <<fmi3GetIntervalFraction>>.

|latexmath:[t_{\mathit{event}}]
...
|latexmath:[T_{\mathit{interval, i}} > 0]
...
is a time-based periodic Clock which ticks with an arbitrary, but fixed interval latexmath:[T_{\mathit{interval}}] starting after an arbitrary latexmath:[T_{\mathit{shift}}], both interval and shift can be changed by the importer.
...
is a time-based periodic Clock which ticks with an arbitrary and changeable interval latexmath:[T_{\mathit{interval}}] starting after an arbitrary latexmath:[T_{\mathit{shift}}], both interval and shift can be changed by the importer.
...
Calling <<fmi3GetShift>> or <<fmi3SetShift>> is not allowed.
...
|latexmath:[T_{\mathit{interval, i}} \geq 0, \qquad i = 1,2,3,{...}]
...
Each Clock latexmath:[k] induces a discrete subsystem.
Its state advances at each tick of latexmath:[k].
Such a system, also called the <<model-partition>> of latexmath:[k], may represent a part of control code, an interrupt service routine of an embedded system, or a discretized part of a plant model.
...
* latexmath:[\mathbf{f}_{\mathit{disc}}] denotes the state transition function and
* latexmath:[\mathbf{f}_{\mathit{event}}] denotes the output function of the <<model-partition>> of latexmath:[k].

When latexmath:[k] is active (i.e., ticking) invoking the FMI functions `fmi3Get{VariableType}` on variables in the output vector latexmath:[\mathbf{y}_k] will trigger the execution of the output function latexmath:[\mathbf{f}_{\mathit{event}}] to compute such variables.
If <<providesEvaluateDiscreteStates>> is `false`, then latexmath:[\mathbf{f}_{\mathit{disc}}] cannot be called explicitly using <<fmi3EvaluateDiscreteStates>>, but will be implicitly executed during latexmath:[\mathbf{f}_{\mathit{event}}].
When <<fmi3EvaluateDiscreteStates>> is called, the discrete-time state vector latexmath:[\mathbf{x}_k] is updated according to state transition function latexmath:[\mathbf{f}_{\mathit{disc}}].

_[Separating_ latexmath:[\mathbf{f}_{\mathit{disc}}] _from_ latexmath:[\mathbf{f}_{\mathit{event}}] _allows manipulation of the current (discrete) state and compute the corresponding outputs, for instance for model-based control applications.]_

The discrete-time variables latexmath:[\mathbf{x}_k] and latexmath:[\mathbf{y}_k] are called clocked variables.
Clocked variables latexmath:[\mathbf{v}_k] can acquire new values and can be queried only when their Clock latexmath:[k] is active, except during <<InitializationMode>>.
...
A <<ClockedState>> is a clocked variable belonging to the discrete-time state vector latexmath:[\mathbf{x}_k], whose value depends on its previous value (i.e., the value computed at the last Clock tick).
...
A Clock latexmath:[k] depends on a Clock or variable latexmath:[v] if a tick or the value of latexmath:[v] may trigger a tick of latexmath:[k] during the same super-dense time instant.
...
For restrictions on when to call which of the following functions for <<table-overview-clocks,each Clock type>>, see <<table-overview-clocks>>.
...
[source, C]
----
include::../headers/fmi3FunctionTypes.h[tag=SetShiftDecimal]
----

[[fmi3SetShiftFraction,`fmi3SetShiftFraction`]]
[source, C]
----
include::../headers/fmi3FunctionTypes.h[tag=SetShiftFraction]
----
...
The retrieved dependency information of one variable becomes invalid as soon as a <<structuralParameter,`structural parameter`>> linked to the variable or to any of its depending variables is set.

==== Selective Computation of Variables [[selective-computation]]
...
This data may be provided in the <<modelDescription.xml>> under element <<ModelStructure>>.
...
When solving algebraic loops in <<EventMode>>, limitations to variable manipulations declared with attribute <<canHandleMultipleSetPerTimeInstant>> must be observed.
...
The precise definitions are given in the mathematical descriptions of Model Exchange (<<state-machine-model-exchange>>), Co-Simulation (<<state-machine-co-simulation>>), and Scheduled Execution (<<state-machine-scheduled-execution>>).
In every state, the general form of the FMU equations is:
...
* latexmath:[\mathbf{v}_{\mathit{unknown}}] is the vector of unknown variables computed in the current mode:
...
Each continuous-time state derivative variable listed as elements <<ContinuousStateDerivative>> in <<ModelStructure>>, if present.

* latexmath:[\mathbf{v}_{\mathit{known}}] is the vector of arguments of function latexmath:[\mathbf{f}] which can be changed by the importer in the current mode.
...
* latexmath:[{\mathbf{v}_{\mathit{rest}}}] is the set of <<input>> variables and discrete-time states of function latexmath:[\mathbf{f}] which cannot be set by the importer in the current mode, but in other modes _[for example, discrete-time <<input,`inputs`>> in <<ContinuousTimeMode>>]_.
...
- _If the FMU is connected with other FMUs, the partial derivatives of the state derivatives and outputs with respect to the continuous-time states and the <<input,`inputs`>> are needed in order to compute the Jacobian for the system of the connected FMUs._
