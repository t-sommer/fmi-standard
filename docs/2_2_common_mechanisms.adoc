...
* FMI callback functions are not allowed to call back into the FMU (<<logMessage,`fmi3LogMessageCallback`>>, <<preemption-support,`fmi3LockPreemptionCallback`>>, <<preemption-support,`fmi3UnlockPreemptionCallback`>>), except <<intermediateUpdate,`fmi3IntermediateUpdateCallback`>> and <<fmi3ClockUpdateCallback>> which lead to the well-defined states <<IntermediateUpdateMode>> and <<ClockUpdateMode>>, respectively, defining which function calls back into the FMU are allowed.
...
Access to operating system resources and services, such as memory, network or file system, must be implemented with special care because the availability of such resources and services is not guaranteed on every target platform and/or simulator.
...
An FMU C-file must include at the beginning a `define` of `FMI3_FUNCTION_PREFIX` with the same value as the value of the <<modelIdentifierME>> attribute defined in `<fmiModelDescription><ModelExchange>`, <<modelIdentifierCS>> attribute defined in `<fmiModelDescription><CoSimulation>` or <<modelIdentifierSE>> attribute defined in `<fmiModelDescription><ScheduledExecution>` together with `pass:[_]` at the end (see <<model-exchange-schema>>, <<co-simulation-schema>>, <<scheduled-execution-schema>>).
...
In case of a static link library, the name of the library is `MyModel.lib` on Windows and `libMyModel.a` on Linux; in other words the <<modelIdentifierME>> attribute is used to create the library name.
...
The name of the library is `MyModel.dll` on Windows or `MyModel.so` on Linux; in other words the <<modelIdentifierME>> attribute is used as library name.
...
Since <<modelIdentifierME>> is used as prefix of a C-function name it must fulfill the restrictions on C-function
names (only letters, digits and/or underscores are allowed).
_[For example, if `modelName = "A.B.C"`, then <<modelIdentifierME>> might be "A_B_C".]_
Since <<modelIdentifierME>> is also used as name in a file system, it must also fulfill the restrictions of the targeted operating system.
...
[source, C]
----
include::examples/snippets.c[tags=ModelStruct1]
    // ...

include::examples/snippets.c[tags=ModelStruct2]
----
...
The FMU may use the return argument <<earlyReturn>> of the <<fmi3DoStep>> function to tell the importer that the FMU returned earlier.
...
For FMUs these points in time are <<event,events>> defined by time-based or triggered <<Clock,Clocks>> for all exposed model partitions of a set of FMUs, or clock ticks signaled by the FMUs.
...
* _On a real-time simulator the scheduler activates the model partitions of the same example whenever the wall clock's time has progressed for 5ms or 10ms respectively.]_
...
A variable, except for Clocks, can be a scalar or a multi-dimensional array.
Clocks are always scalar.
...
In addition to those state-specific restrictions, setting and getting of <<clocked-variable,clocked variables>> is only allowed during <<EventMode>> when their respective Clocks are active, and during <<InitializationMode>> irrespective of Clock actication status.
...
_[The passing of `nValues` is, strictly speaking, passing redundant information:
...
It is added to ensure memory safety and other sanity checks to be preserved even in the face of potential divergence of array configuration.]_
...
The variable's XML attribute <<intervalVariability>> declares the type of Clock, described in detail after the table.
...
2+h|XML attributes
.2+h|Related API calls and arguments
.2+h|Example
...
.4+|[[periodic]]periodic
.4+|<<input>>
|[[constant-intervalVariability,`constant`]]`constant`
...
<<fmi3ActivateModelPartition>> in <<ClockActivationMode>>,
<<fmi3SetInterval>>, <<fmi3GetInterval>> and <<fmi3GetShift>> in <<InitializationMode>>
...
In Scheduled Execution the FMU has to inform the importer by calling <<fmi3ClockUpdateCallback>> that the Clock is about to tick
...
is a time-based Clock whose next interval is not yet known right after the Clock just ticked, forcing the importer to call <<fmi3GetInterval>> in every <<EventMode>> and in every <<ClockUpdateMode>>.
...
_[<<outputClock,Output Clocks>> are intended to enable an FMU to provide a trigger to the outside world, e.g. to a <<triggered,triggered input Clock>> of another FMU._
...
It's state advances at each tick of latexmath:[\mathbf{k}].
Such a system, also called the <<model-partition>> of latexmath:[\mathbf{k}], is written as:
...
_[Such a model partition may represent part of control code or an interrupt service routine of an embedded system, or a discretized part of a plant model._
_The name "model partition" refers to the fact that a model inside an FMU may declare multiple Clocks, each with its own partition of the overall model.]_
...
_[Separating_ latexmath:[\mathbf{f}_{disc}] _from_ latexmath:[\mathbf{f}_{event}] _allows manipulation of the current (discrete) state and compute the corresponding outputs.]_
...
_[E.g. if an <<outputClock>> depends on an <<inputClock>>, then Clock ticks of the <<inputClock>> in <<EventMode>> or <<ClockActivationMode>> may create <<outputClock>> ticks during this event handling.]_
...
The FMU signals that a <<countdown>> Clock is about to tick by calling <<fmi3ClockUpdateCallback>>.
...
Note, that the association between <<clocked-variable,clocked variables>> and their <<Clock,Clocks>> is defined by the attribute <<clocks>> in `<fmiModelDescription><ModelVariables>`.
