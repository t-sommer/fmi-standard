...
* FMI callback functions are not allowed to call back into the FMU (<<logMessage,fmi3CallbackLogMessage>>, <<preemption-support,fmi3CallbackLockPreemption>>, <<preemption-support,fmi3CallbackUnlockPreemption>>), except <<intermediateUpdate,fmi3CallbackIntermediateUpdate>> which leads to the well-defined state <<IntermediateUpdateMode>> defining which function calls back into the FMU are allowed.
...
Importers of source code FMUs may define `FMI3_FUNCTION_PREFIX` before including `fmi3Functions.h` _[either as a definition in the source code or by passing it as an option to the compiler]_.
...
fmi3SetTimeTYPE *myname_setTime;
// load symbol "fmi3SetTime" from the shared library
...
- If the implementation is provided as source code: +
The constructed function name is `<FMI3_FUNCTION_PREFIX>fmi3GetFloat64`.

- If the implementation is provided as a static library: +
...
In case of a static link library, the name of the library is `model.lib` on Windows and `libmodel.a` on Linux and Darwin.

- If the implementation is provided a shared library: +
...
The name of the library is `model.dll` on Windows, `model.so` on Linux, and `model.dylib` on Darwin.
...
_In this tool coupling case one shared library can be used for all models due to no function prefixing.]_
